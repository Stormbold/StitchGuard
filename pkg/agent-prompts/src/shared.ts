...
export function generateProjectAgentsMd(result: CompareResult): string {
  return `# AGENTS.md

This file was generated by StitchGuard to guide AI coding agents during visual repair.

## Task

Make the current UI match the target screenshot more closely.

## Visual Match

${formatScorePercent(result.score)}

## Main Differences

${formatFindingsList(result.findings).map((item) => `- ${item}`).join('\n')}

## Rules

${CONSERVATIVE_RULES.map((rule) => `- ${rule}`).join('\n')}
`;
}
